...
namespace Jackie    //2021.május Szoftverfejlesztő OKJ központi feladat megoldása
{
    class Program
    {
        ...
        {
            Beolvasas(); //jackie.txt beolvasása
            F3MO(); //adatsorok száma
            F4MO(); //Jackie Steward melyik évben indult el a legtöbb versenyen Linq-val
            F5MO(); //Jackie Steward számára melyik évtized mennyire volt sikeres a megnyert versenyek száma alapján. Linq-val
            F6MO(); //jackie.html létrehozása táblázatos formában jelenjen meg a versenyzés éve, versenyek és győzelmek száma.
            Console.ReadLine();
        }
        ...
        private static void F6MO()
        {
            try
            {
                List<Versenyzo> sorbarendezett = adatok.OrderByDescending(x => x.Ev).ToList();// Létrehozok egy új  csökkenő sorba rendezem az évszámok szerint
